...
어릴 때부터 해온 메이플스토리에서 아르카나 돌의 정령의 독특한 말투가 언제나 인상 깊었습니다.<br>
한 번쯤 직접 구현해보고 싶다는 생각이 있었고, 이번 프로젝트를 통해 도전해보게 됐습니다.<br><br>
처음엔 프롬프트만으로 시작했지만 세계관을 모르면 엉뚱한 답을 내놓는 한계를 금방 마주했고,<br>
이를 해결하기 위해 나무위키 데이터를 직접 크롤링해 RAG를 구축했습니다.<br>
이후 단순 대화를 넘어 게임 내 정보를 스스로 조회하고 행동하는 에이전트로 발전시켰고,<br>
FastAPI 서버와 Unity 2D 씬을 연동해 실제 NPC처럼 동작하는 데모까지 완성했습니다.
...
3. **Gradio UI 시작**:
```bash
python main.py          # 크롤링 및 ChromaDB 세팅 후 Gradio 시작
```

4. **FastAPI 서버 실행 (Unity 연동용)**:
```bash
...
- RAG 없이 테스트<br>
프롬프트만으로 돌의 정령 캐릭터를 구현하고 난 후, 메이플스토리의 세계관 기반 질문을 할때 세계관을 이탈한 답을 내거나 말투가 깨지는 경우가 종종 발생.<br>
나무위키에서 아르카나 스토리 데이터를 직접 크롤링해 ChromaDB에 구축한 뒤로는 퀘스트, 등장인물 관련 질문에도 정확하게 답하기 시작.
<br><br>
- 모델 nano VS mini<br>
처음엔 비용 절감을 위해 gpt-5.4-nano로 먼저 테스트해 보았고, <br>
말투의 규칙 준수가 불안정하여 "좋달람"과 같은 잘못된 어미가 지속적으로 나왔음. <br>
...
- Pydantic으로 응답 무결성 강화<br><br>
***** 두 가지 상황을 고려하여 도입하였습니다. ******<br><br>
...
`test_evaluation.py`로 20개 질문을 기준으로 두 모델을 비교 평가. (평가 기준: 담/람 어미 유지 여부, 금지 어미 포함 여부)

| 항목 | gpt-5.4-nano | gpt-5.4-mini |
|------|:---:|:---:|
| 말투 유지율 | 100% (20/20) | 100% (20/20) |
| 금지 어미 이탈 | 0건 | 0건 |
| 평균 응답 시간 | 1.17s | 1.02s |
| 평균 출력 토큰 | 28.4 | 26.0 |
| 1,000회 예상 비용 | $0.27 | $0.99 |

### **분석**<br>
Pydantic 검증 + 최대 3회 재시도 구조 덕분에 두 모델 모두 말투 유지율 100% 달성.<br>
nano가 비용은 약 3.7배 저렴하나, 검증 없이 테스트를 했을때 `그래담`, `지담` 등 금지 어미가 반복 발생해 재시도 횟수가 증가하는 경향.<br>
응답 속도는 mini가 오히려 좀더 빠르고(1.02s vs 1.17s), 출력 토큰도 더 적어 응답이 간결한 점도 mini 채택한 이유임.

---

## 추후 개선 방향 
- 코드 구조 정리 (현재 Python 파일들이 기능별로 분리되어 있지 않고 루트에 나열된 상태라 패키지 구조로 리팩토링 필요)
